...
<head>
    ...
    <style>
        * {
        ...
        .todo-card { background: #ffffff; border-radius: 28px; border: 1px solid #eef2f9; overflow: hidden; cursor: grab; position: relative; }
        ...
        .edit-category-btn, .delete-category-btn, .move-category-btn {
            background: none;
            border: none;
            cursor: pointer;
            font-size: 0.75rem;
        ...
        .move-category-btn:hover { background: #dce5ec; color: #2c7a5e; }
        ...
        .week-table { min-width: 800px; border-collapse: collapse; width: 100%; }
        ...
        .habit-name-cell { font-weight: 600; font-size: 0.9rem; white-space: nowrap; }
        ...
        /* 移动分类下拉菜单浮层 */
        .move-menu { position: absolute; right: 30px; top: 40px; background: white; border-radius: 24px; box-shadow: 0 8px 20px rgba(0,0,0,0.15); border: 1px solid #e2e8f0; z-index: 200; min-width: 160px; overflow: hidden; max-height: 280px; overflow-y: auto; }
        .move-menu-item { padding: 10px 16px; font-size: 0.85rem; cursor: pointer; transition: background 0.1s; white-space: nowrap; }
        .move-menu-item:hover { background: #eef2f6; }
        .move-menu-divider { height: 1px; background: #eef2f6; margin: 4px 0; }
        .move-menu-title { padding: 8px 16px; font-size: 0.7rem; color: #8aaeb8; font-weight: 500; background: #fafcff; border-bottom: 1px solid #eef2f8; }
        .drag-handle { cursor: grab; opacity: 0.5; font-size: 1rem; margin-right: 6px; display: inline-block; }
        .highlight-default { background: #e1f5fe; transition: 0.2s; }
    </style>
</head>
<body>
<div class="app">
...
<script>
    ...
    function getThisSundayStr() {
        const today = new Date();
        const day = today.getDay();
        const sunday = new Date(today);
        sunday.setDate(today.getDate() + (day === 0 ? 0 : 7 - day));
        return `${sunday.getFullYear()}-${String(sunday.getMonth()+1).padStart(2,'0')}-${String(sunday.getDate()).padStart(2,'0')}T23:59:59`;
    }
    function getThisMonthEndStr() {
        const today = new Date();
        const lastDay = new Date(today.getFullYear(), today.getMonth() + 1, 0);
        return `${lastDay.getFullYear()}-${String(lastDay.getMonth()+1).padStart(2,'0')}-${String(lastDay.getDate()).padStart(2,'0')}T23:59:59`;
    }
    ...
    function getArchivedCategories() {
        ...
        todos.forEach(todo => {
            if(todo.completed && todo.completedAt) {
                const dateStr = todo.completedAt.slice(0,10);
                if(!archiveMap.has(dateStr)) archiveMap.set(dateStr, []);
                archiveMap.get(dateStr).push(todo);
            }
        });
        return archiveMap;
    }
    ...
    function loadFilterCategoriesOrder() {
        ...
        filterCategories = [
            ...
            { id: 'default', name: '默认', type: 'fixed', deletable: false, editableName: true, order: 1 },
            { id: 'week', name: '本周', type: 'fixed', deletable: false, editableName: true, order: 2 },
            { id: 'month', name: '本月', type: 'fixed', deletable: false, editableName: true, order: 3 },
            { id: 'active', name: '未完成', type: 'fixed', deletable: false, editableName: true, order: 4 },
            { id: 'completed', name: '已完成', type: 'fixed', deletable: false, editableName: true, order: 5 }
        ];
        saveFilterCategoriesOrder();
    }
    function saveFilterCategoriesOrder() { localStorage.setItem(STORAGE_KEY + '_filter_order', JSON.stringify(filterCategories)); }
    ...
    function moveTodoToCategory(todoId, targetCategoryName) {
    ...
    function moveTodoToSmartPeriod(todoId, periodType) {
        const todo = todos.find(t => t.id == todoId);
        if(!todo) return;
        let newDeadline = null;
        if(periodType === 'week') newDeadline = getThisSundayStr();
        else if(periodType === 'month') newDeadline = getThisMonthEndStr();
        else if(periodType === 'default') newDeadline = null;
        todo.deadline = newDeadline;
        todo.endDate = null;
        saveToLocal();
        renderAll();
    }
    function moveTodoToCompletionState(todoId, targetCompleted) {
        const todo = todos.find(t => t.id == todoId);
    ...
    function showMoveMenu(todoId, currentCat, btnElement) {
        const menu = document.createElement('div');
        menu.className = 'move-menu';
        
        const smartSection = document.createElement('div');
        smartSection.innerHTML = '<div class="move-menu-title">📅 按时间</div>';
        const smartOptions = [
            { label: '📅 移至本周', type: 'week' },
            { label: '📆 移至本月', type: 'month' },
            { label: '📌 移至默认', type: 'default' }
        ];
        smartOptions.forEach(opt => {
            ...
            item.onclick = (e) => {
                e.stopPropagation();
                moveTodoToSmartPeriod(todoId, opt.type);
                document.body.removeChild(menu);
            };
            smartSection.appendChild(item);
        });
        menu.appendChild(smartSection);
        
        const stateSection = document.createElement('div');
        stateSection.innerHTML = '<div class="move-menu-title">✅ 按状态</div>';
        const stateOptions = [
            { label: '✓ 标记为已完成', completed: true },
            { label: '○ 标记为未完成', completed: false }
        ];
        stateOptions.forEach(opt => {
            ...
            item.onclick = (e) => {
                e.stopPropagation();
                moveTodoToCompletionState(todoId, opt.completed);
                document.body.removeChild(menu);
            };
            stateSection.appendChild(item);
        });
        menu.appendChild(stateSection);
        
        let allCats = new Set();
        allCats.add('默认');
        customCategories.forEach(c => allCats.add(c.name));
        const categoryList = Array.from(allCats).filter(c => c !== currentCat);
        if(categoryList.length > 0) {
            const catSection = document.createElement('div');
            catSection.innerHTML = '<div class="move-menu-title">📁 按分类</div>';
            categoryList.forEach(cat => {
                const item = document.createElement('div');
                item.className = 'move-menu-item';
                item.innerText = `📁 移至 ${cat}`;
                item.onclick = (e) => {
                    e.stopPropagation();
                    moveTodoToCategory(todoId, cat);
                    document.body.removeChild(menu);
                };
        ...
        const closeMenu = (e) => { if(menu.parentNode) document.body.removeChild(menu); document.removeEventListener('click', closeMenu); };
    ...
    function addTodo(title, deadlineISO = null, endISO = null, category = null) { if (!title.trim()) return; const mo = todos.length ? Math.max(...todos.map(t => t.order ?? 0)) : -1; todos.push({ id: Date.now(), title: title.trim(), completed: false, deadline: deadlineISO || null, endDate: endISO || null, createdAt: Date.now(), order: mo + 1, category: category || '默认', completedAt: null }); saveToLocal(); renderAll(); }
    function toggleTodoComplete(todoId) { const todo = todos.find(t => t.id == todoId); if (todo) { todo.completed = !todo.completed; if(todo.completed) todo.completedAt = new Date().toISOString(); else todo.completedAt = null; saveToLocal(); renderAll(); } }
    ...
    function renderMonthDoubleColumn() {
        ...
        let lastValidIndex = 41;
        for(let i=41;i>=0;i--) { if(monthDates[i].getMonth() === month) { lastValidIndex = i; break; } }
        const trimmedDates = monthDates.slice(0, lastValidIndex+1);
        const totalCells = trimmedDates.length;
        ...
        habits.forEach(habit => {
            ...
            for (let row = 0; row < rows; row++) {
                ...
                for (let col = 0; col < 7; col++) {
                    ...
                    const d = trimmedDates[idx]; const ds = formatYMD(d); const isCurrentMonth = (d.getMonth() === month);
    ...
    function renderTodayDouble() { 
        ...
        let html = `<div class="stats-summary"><div>✅ 今日完成: ${completedCount}/${habits.length} 项习惯</div><div>📋 待办剩余: ${todoRemain}</div></div><div class="two-columns" id="todaySortableGrid">`; 
        habits.forEach(habit => { 
    ...
    function renderWeekMatrix() { 
        ...
        let totalPossible = filtered.length * totalDays, totalCompleted = 0; 
        ...
        let html = `<div class="stats-summary"><div>📆 本周打卡进度: ${totalCompleted}/${totalPossible}</div><div class="stat-bar-wrapper"><div class="stat-bar-bg"><div class="stat-bar-fill" style="width:${weekProgress}%;"></div></div><div class="stat-value">${weekProgress}%</div></div><div>📋 待办剩余: ${todos.filter(t=>!t.completed).length}</div></div>
                    <div class="calendar-nav"><button id="prevWeekBtn">◀ 上周</button><span>${weekStartStr.slice(5)} ~ ${weekEndStr.slice(5)}</span><button id="nextWeekBtn">下周 ▶</button></div>
                    <div class="filter-row" id="weekFilterContainer"></div>
                    <div class="week-matrix"><table class="week-table"><thead><tr><th>习惯</th>`;
        for (let day of weekDays) { 
            const md = `${day.getMonth()+1}/${day.getDate()}`; 
            const weekLabel = ['一','二','三','四','五','六','日'][day.getDay()===0?6:day.getDay()-1]; 
            html += `<th>${md}<br><span style="font-size:0.6rem;">${weekLabel}</span></th>`; 
        } 
        html += `<th>打卡天数</th><th>本周合计</th><th>单位</th></tr></thead><tbody id="weekSortableBody">`;
        for (let habit of filtered) { 
            let weeklySum = 0, weekDaysWithData = 0; 
            let weekCells = ''; 
            for (let day of weekDays) { 
                ...
                if (habit.type === 'record') { 
                    ...
                    weekCells += `<td><div class="week-status-dot ${hasData ? 'complete' : ''}" data-habit="${habit.id}" data-date="${ds}">${hasData ? '✓' : '○'}</div><input type="text" class="week-value-input ${hasData ? 'complete' : ''}" data-habit="${habit.id}" data-date="${ds}" value="${escapeHtml(String(val))}"></td>`; 
                } else { 
                    val = habit.values?.[ds] || 0; 
                    weeklySum += val; 
                    weekCells += `<td><div class="week-status-dot ${hasData ? 'complete' : ''}" data-habit="${habit.id}" data-date="${ds}">${hasData ? '✓' : '○'}</div><input type="number" step="any" class="week-value-input ${hasData ? 'complete' : ''}" data-habit="${habit.id}" data-date="${ds}" value="${val}"></td>`; 
                } 
            } 
            const unit = habit.unit || ''; 
            const displayTotal = habit.type === 'accumulate' ? weeklySum : '-'; 
            html += `<tr data-habit-id="${habit.id}">
                        <td class="habit-name-cell"><span class="drag-handle">☰</span> <span class="habit-name-editable" data-habit-id="${habit.id}">${escapeHtml(habit.name)}</span><button class="delete-btn" data-id="${habit.id}" style="margin-left:8px;">🗑️</button></td>
                        ${weekCells}
                        <td>${weekDaysWithData}/${totalDays}</td>
                        <td>${displayTotal}</td>
                        <td><input type="text" class="unit-input" data-habit="${habit.id}" value="${escapeHtml(unit)}"></td>
                     </tr>`; 
        } 
    ...
    function renderAllStats() { 
        ...
        let habitStats = habits.map(h => { 
            let total = 0, days = 0; 
            if (h.type === 'record') { 
                if (h.records) days = Object.values(h.records).filter(v => v && v !== '').length; 
            } else { 
                if (h.values) total = Object.values(h.values).reduce((a, b) => a + b, 0); 
                if (h.checkins) days = Object.values(h.checkins).filter(v => v === true).length; 
            } 
            totalAllDays += days; 
        ...
        let html = `<div class="stats-summary"><div>🏆 总打卡天数: ${totalAllDays} 天</div><div>📋 待办剩余: ${todos.filter(t => !t.completed).length}</div></div><div style="margin-top:12px;"><div style="font-weight:600; margin-bottom:16px;">📊 各习惯统计</div>`; 
        habitStats.forEach((hs) => { 
            ...
            html += `<div class="stat-habit-item"><div class="stat-habit-name">${typeIcon} ${escapeHtml(hs.name)}</div><div class="stat-habit-data"><span class="stat-total">${totalText}</span><span class="stat-days">📅 ${hs.days} 天</span></div></div><div class="bar-container"><div class="bar-label">${escapeHtml(hs.name)}</div><div class="bar-bg"><div class="bar-fill" style="width:${barPercent}%;">${barValue}</div></div><div class="bar-value">${barValue}</div></div>`; 
        }); 
        if (habitStats.length === 0) html += `<div class="empty-state">暂无习惯数据</div>`; 
        html += `</div>`; 
        return html; 
    }
    ...
    function filterTodosByCategory(filter) { 
        ...
        if (filter === 'active') filtered = filtered.filter(t => !t.completed);
            ...
            filtered = filtered.filter(t => !t.completed && t.deadline && new Date(t.deadline) >= startOfWeek && new Date(t.deadline) <= endOfWeek); 
        }
            ...
            filtered = filtered.filter(t => !t.completed && t.deadline && new Date(t.deadline) >= startOfMonth && new Date(t.deadline) <= endOfMonth); 
        }
        else if (filter === 'default') { 
            const today = new Date(), endOfMonth = new Date(today.getFullYear(), today.getMonth() + 1, 0, 23, 59, 59, 999); 
            // 修复: 默认分类统计所有未完成的、且没有截止日期或截止日期晚于本月最后一天的任务
            filtered = filtered.filter(t => !t.completed && (!t.deadline || new Date(t.deadline) > endOfMonth)); 
        }
        ...
        else if (filter.startsWith('archive_')) { 
            const dateStr = filter.substring(8); 
            filtered = todos.filter(t => t.completed && t.completedAt && t.completedAt.slice(0,10) === dateStr); 
        }
        return filtered; 
    }
    ...
    function renderTodosByCategory() {
        ...
        const getWeekTodoCount = () => {
            ...
            endOfWeek.setDate(startOfWeek.getDate() + 6); 
            endOfWeek.setHours(23,59,59,999); 
            const weekTodos = todos.filter(t => !t.completed && t.deadline && new Date(t.deadline) >= startOfWeek && new Date(t.deadline) <= endOfWeek);
            return { total: weekTodos.length, completed: weekTodos.filter(t => t.completed).length };
        };
        const getMonthTodoCount = () => {
            const today = new Date(), startOfMonth = new Date(today.getFullYear(), today.getMonth(), 1), endOfMonth = new Date(today.getFullYear(), today.getMonth() + 1, 0, 23, 59, 59, 999);
            const monthTodos = todos.filter(t => !t.completed && t.deadline && new Date(t.deadline) >= startOfMonth && new Date(t.deadline) <= endOfMonth);
            return { total: monthTodos.length, completed: monthTodos.filter(t => t.completed).length };
        };
        const getDefaultTodoCount = () => {
            const today = new Date(), endOfMonth = new Date(today.getFullYear(), today.getMonth() + 1, 0, 23, 59, 59, 999);
            // 修复: 默认分类统计所有未完成的、且没有截止日期或截止日期晚于本月最后一天的任务
            const defaultTodos = todos.filter(t => !t.completed && (!t.deadline || new Date(t.deadline) > endOfMonth));
            return { total: defaultTodos.length, completed: defaultTodos.filter(t => t.completed).length };
        };
        const getCustomCategoryCount = (catName) => {
            const catTodos = todos.filter(t => (t.category || '默认') === catName);
            return { total: catTodos.length, completed: catTodos.filter(t => t.completed).length };
        };
        
        const weekCount = getWeekTodoCount();
        const monthCount = getMonthTodoCount();
        const defaultCount = getDefaultTodoCount();
        
        let filterBarHtml = `<div id="filterSortableContainer" style="display:flex; flex-wrap:wrap; gap:8px; align-items:center;">`;
        
        filterBarHtml += `<div class="filter-todo-wrapper" data-filter-id="all"><span class="filter-drag-icon">☰</span><button class="filter-todo-btn ${todoFilter === 'all' ? 'active' : ''}" data-filter="all">全部 (${allTodos.length})</button></div>`;
        filterBarHtml += `<div class="filter-todo-wrapper" data-filter-id="default"><span class="filter-drag-icon">☰</span><button class="filter-todo-btn ${todoFilter === 'default' ? 'active' : ''}" data-filter="default">默认 (${defaultCount.completed}/${defaultCount.total})</button></div>`;
        filterBarHtml += `<div class="filter-todo-wrapper" data-filter-id="week"><span class="filter-drag-icon">☰</span><button class="filter-todo-btn ${todoFilter === 'week' ? 'active' : ''}" data-filter="week">本周 (${weekCount.completed}/${weekCount.total})</button></div>`;
        filterBarHtml += `<div class="filter-todo-wrapper" data-filter-id="month"><span class="filter-drag-icon">☰</span><button class="filter-todo-btn ${todoFilter === 'month' ? 'active' : ''}" data-filter="month">本月 (${monthCount.completed}/${monthCount.total})</button></div>`;
        filterBarHtml += `<div class="filter-todo-wrapper" data-filter-id="active"><span class="filter-drag-icon">☰</span><button class="filter-todo-btn ${todoFilter === 'active' ? 'active' : ''}" data-filter="active">未完成 (${allTodos.filter(t=>!t.completed).length})</button></div>`;
        
        const completedTodos = allTodos.filter(t => t.completed);
        filterBarHtml += `<div class="filter-todo-wrapper" data-filter-id="completed"><span class="filter-drag-icon">☰</span><button class="filter-todo-btn ${todoFilter === 'completed' ? 'active' : ''}" data-filter="completed">已完成 (${completedTodos.length}/${allTodos.length})</button></div>`;
        
        customCategories.forEach(cat => {
            const catCount = getCustomCategoryCount(cat.name);
            filterBarHtml += `<div class="filter-todo-wrapper" data-filter-id="cat_${cat.id}"><span class="filter-drag-icon">☰</span><button class="filter-todo-btn ${todoFilter === `cat_${cat.id}` ? 'active' : ''}" data-filter="cat_${cat.id}">${escapeHtml(cat.name)} (${catCount.completed}/${catCount.total})</button><button class="edit-category-btn" data-filter-id="cat_${cat.id}" title="编辑分类名">✏️</button><button class="delete-category-btn" data-filter-id="cat_${cat.id}" title="删除分类">🗑️</button></div>`;
        });
        ...
        document.querySelectorAll('.filter-todo-btn').forEach(btn => { btn.addEventListener('click', (e) => { todoFilter = btn.getAttribute('data-filter'); renderAll(); }); });
        document.querySelectorAll('.edit-category-btn').forEach(btn => { btn.addEventListener('click', (e) => { e.stopPropagation(); const fid = btn.getAttribute('data-filter-id'); const cat = filterCategories.find(c => c.id === fid); if(cat) { const newName = prompt("输入分类名称", cat.name); if(newName && newName.trim()) renameFilterCategory(fid, newName.trim()); } }); });
        document.querySelectorAll('.delete-category-btn').forEach(btn => { btn.addEventListener('click', (e) => { e.stopPropagation(); const fid = btn.getAttribute('data-filter-id'); if(confirm("确定删除此分类？自定义分类下的待办将移至「默认」")) { const cat = filterCategories.find(c => c.id === fid); if(cat && cat.type === 'custom') { todos.forEach(t => { if(t.category === cat.name) t.category = '默认'; }); deleteFilterCategory(fid); saveToLocal(); } else { alert("固定分类不可删除"); } } }); });
        ...
        if(container && typeof Sortable !== 'undefined') { 
            new Sortable(container, { handle: '.filter-drag-icon', animation: 200, onEnd: () => { 
                const newOrderWrappers = document.querySelectorAll('.filter-todo-wrapper');
                let newOrder = [];
                newOrderWrappers.forEach(w => { const fid = w.getAttribute('data-filter-id'); if(fid && !fid.startsWith('archive_')) newOrder.push(fid); });
                let changed = false;
                newOrder.forEach((id, idx) => { const catIdx = filterCategories.findIndex(c => c.id === id); if(catIdx !== -1 && filterCategories[catIdx].order !== idx) { filterCategories[catIdx].order = idx; changed = true; } });
                if(changed) { filterCategories.sort((a,b) => a.order - b.order); saveFilterCategoriesOrder(); renderTodosByCategory(); } 
            } }); 
        }
        ...
        if(todoFilter === 'completed') {
            const archiveMap = getArchivedCategories();
            const sortedDates = Array.from(archiveMap.keys()).sort().reverse();
            sortedDates.forEach(dateStr => { groups.set(`📅 ${dateStr}`, archiveMap.get(dateStr)); });
            if(sortedDates.length === 0) groups.set('暂无已完成事项', []);
        } else {
        ...
        for(let [catName, catTodos] of groups.entries()) {
            if(catTodos.length === 0 && catName === '暂无已完成事项') { finalHtml += `<div class="empty-state">📭 暂无已完成任务</div>`; continue; }
            ...
            catTodos.sort((a,b) => (a.order??0)-(b.order??0)).forEach(todo => {
                ...
                } else deadlineHtml='⏳ 默认';
                finalHtml += `<div class="todo-card" data-todo-id="${todo.id}" data-category="${escapeHtml(catName)}"><div class="card-header"><div><span class="drag-icon">☰</span> 待办</div><div><button class="move-category-btn" data-todo-id="${todo.id}" style="margin-right:4px;" title="移动分类">📂</button><button class="delete-btn todo-del" data-id="${todo.id}">🗑️</button></div></div><div class="todo-card-content"><div class="todo-title-row"><input type="checkbox" class="todo-check" data-id="${todo.id}" ${todo.completed?'checked':''}><div class="todo-title-text editable-todo" data-todo-id="${todo.id}">${escapeHtml(todo.title)}</div></div><div><span class="todo-deadline-badge ${dc}" data-todo-id="${todo.id}" style="cursor:pointer;">${deadlineHtml}</span></div></div></div>`;
            });
            finalHtml += `</div></div>`;
        }
        if(filteredTodos.length===0 && todoFilter !== 'completed') finalHtml='<div class="empty-state">📭 暂无待办任务</div>';
        finalHtml += `</div>`;
        ...
        document.querySelectorAll('.move-category-btn').forEach(btn => { btn.addEventListener('click', (e) => { e.stopPropagation(); const todoId = parseInt(btn.getAttribute('data-todo-id')); const todo = todos.find(t=>t.id===todoId); const currentCat = todo.category || '默认'; showMoveMenu(todoId, currentCat, btn); }); });
    }

    function bindValueEvents() { 
        document.querySelectorAll('.status-dot, .week-status-dot, .month-status-dot').forEach(dot => { 
            dot.addEventListener('click', (e) => { const hid = dot.getAttribute('data-habit'); const date = dot.getAttribute('data-date'); toggleCheckIn(hid, date); }); 
        }); 
        document.querySelectorAll('.quantify-input, .week-value-input, .month-value-input').forEach(inp => { 
            inp.addEventListener('change', (e) => { const hid = inp.getAttribute('data-habit'); const date = inp.getAttribute('data-date'); setHabitValue(hid, date, inp.value); }); 
        }); 
        document.querySelectorAll('.quantify-unit, .unit-input').forEach(inp => { 
            inp.addEventListener('change', (e) => { const hid = inp.getAttribute('data-habit'); setHabitUnit(hid, inp.value); }); 
        }); 
        document.querySelectorAll('.type-option').forEach(opt => { 
            opt.addEventListener('click', (e) => { const hid = opt.getAttribute('data-habit'); const type = opt.getAttribute('data-type'); setHabitType(hid, type); }); 
        }); 
    }
    ...
    function renderDynamic() { 
        ...
        bindValueEvents(); 
        document.querySelectorAll('.editable-name, .habit-name-editable').forEach(el=>{ 
            el.addEventListener('click',(e)=>{ e.stopPropagation(); const hid = el.getAttribute('data-habit-id'); const cur = el.innerText; makeEditable(el, cur, (nv)=>updateHabitName(hid,nv)); }); 
        }); 
        if(currentTab==='day'){ 
    ...
    function loadData(){ 
        ...
        if(!todos.length) todos=[{id:1001,title:'整理周报',completed:false,deadline:null,endDate:null,createdAt:Date.now(),order:0, category:'默认', completedAt:null}]; 
        ...
        todos.forEach(t=>{ if(t.order===undefined) t.order=0; if(t.deadline===undefined) t.deadline=null; if(t.endDate===undefined) t.endDate=null; if(t.category===undefined) t.category='默认'; if(t.completedAt===undefined) t.completedAt=null; }); 
